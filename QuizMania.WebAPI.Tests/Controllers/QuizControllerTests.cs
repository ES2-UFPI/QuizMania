...
namespace QuizMania.WebAPI.Tests.Controllers
{
    ...
    partial class QuizControllerTests : DbContextTestBase
    {
        ...
        [TestCase]
        public async Task Test_GetQuizzes_All()
        {
            ...
            var quizzes = okResult.Value as ICollection<QuizReadDTO>;
            Assert.NotNull(quizzes);

            Assert.AreEqual(quizCount, quizzes.Count);
        }
        ...
        [TestCase(999)]
        public async Task Test_GetQuiz_NonExistent(long value)
        {
            var context = GetQuizTestsDatabaseContext();
            var controller = new QuizController(new QuizService(new QuizRepository(context), new CharacterRepository(context),
                             new CharacterService(new CharacterRepository(context), new ItemRepository(context), Mapper), Mapper));
        ...
        [TestCase(1)]
        [TestCase(2)]
        public async Task Test_GetQuiz_Existent(long value)
        {
            var context = GetQuizTestsDatabaseContext();
            var controller = new QuizController(new QuizService(new QuizRepository(context), new CharacterRepository(context),
                             new CharacterService(new CharacterRepository(context), new ItemRepository(context), Mapper), Mapper));
        ...
        public async Task Test_DeleteQuestion_Success(long characterId, long quizId, long questionId)
        {
            ...
            var actionResult = await controller.DeleteQuestion(new DeleteQuestionRequestDTO { CharacterId = characterId, 
                                                                                              QuizId = quizId, 
